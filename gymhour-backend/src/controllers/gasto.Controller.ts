...
const createGasto = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(201).json({ message: 'Gasto creado correctamente', data: gasto });
      ...
      respondUnexpected(res, error, "guardar el gasto");
  }
};
...
const getAllGastos = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(200).json({
      ...
      respondUnexpected(res, error, "cargar los gastos");
  }
};
...
const updateGasto = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (isNaN(id)) {
      res.status(400).json({ message: 'No pudimos identificar el gasto. Actualizá la página e intentá de nuevo.' });
      return;
    }
    ...
    if (error?.code === 'P2025') {
      res.status(404).json({ message: 'No encontramos ese gasto.' });
      return;
    }
    respondUnexpected(res, error, "actualizar el gasto");
  }
};
...
const deleteGasto = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    respondUnexpected(res, error, "eliminar el gasto");
  }
};
